...
// ═══════════════════════════════════════════════════════════════════════════════════
// KEYWORD EXTRACTOR CONFIGURATION - ELITE V18.0
// ═══════════════════════════════════════════════════════════════════════════════════

var KEYWORD_EXTRACTOR_CONFIG = {
  // Extraction limits - UPGRADED for comprehensive coverage
  MAX_PRIMARY_KEYWORDS: 30,      // Increased from 20
  MAX_SECONDARY_KEYWORDS: 50,    // Increased from 30
  MAX_SEMANTIC_KEYWORDS: 60,     // Increased from 40
  MAX_LONGTAIL_KEYWORDS: 80,     // Increased from 50
  MAX_PAA_QUESTIONS: 30,         // Increased from 20
  MAX_FAQ_QUESTIONS: 40,         // Increased from 30
  
  // N-gram settings - More comprehensive extraction
  MIN_NGRAM: 1,
  MAX_NGRAM: 6,                  // Increased from 5 for longer phrases
  MIN_FREQUENCY: 2,
  MIN_WORD_LENGTH: 3,
  
  // Keyword classification thresholds
  PRIMARY_THRESHOLD: 4,          // Lowered for better detection
  SECONDARY_THRESHOLD: 2,        // Lowered for more coverage
  LONGTAIL_MIN_WORDS: 3,
  
  // NEW: TF-IDF weighting for relevance scoring
  ENABLE_TFIDF: true,
  TFIDF_BOOST_TITLE: 3.0,        // Title keywords get 3x boost
  TFIDF_BOOST_H1: 2.5,           // H1 keywords get 2.5x boost
  TFIDF_BOOST_H2: 2.0,           // H2 keywords get 2x boost
  TFIDF_BOOST_H3: 1.5,           // H3 keywords get 1.5x boost
  TFIDF_BOOST_BOLD: 1.3,         // Bold/strong text gets 1.3x boost
  TFIDF_BOOST_META: 2.0,         // Meta description keywords
  
  // NEW: Entity extraction patterns
  ENTITY_PATTERNS: {
    product: /\b(?:software|tool|platform|app|application|service|solution|system)\b/gi,
    brand: /\b(?:[A-Z][a-z]+(?:\s+[A-Z][a-z]+)*)\b/g,
    technology: /\b(?:AI|ML|API|SaaS|cloud|automation|analytics|algorithm)\b/gi,
    industry: /\b(?:marketing|finance|healthcare|ecommerce|retail|technology|education)\b/gi
  },
  
  // NEW: Semantic clustering configuration
  SEMANTIC_CLUSTERING: {
    enabled: true,
    minClusterSize: 3,
    maxClusters: 15,
  ...
  STOP_WORDS: new Set([
    ...
    'privacy', 'policy', 'terms', 'conditions', 'copyright', 'rights',
    'reserved', 'cookie', 'cookies', 'accept', 'decline', 'close',
    'share', 'facebook', 'twitter', 'linkedin', 'instagram', 'email',
    'print', 'save', 'download', 'upload', 'submit', 'cancel', 'okay',
    'loading', 'please', 'wait', 'error', 'success', 'warning', 'info'
  ]),
  
  // Intent patterns - EXPANDED with more signals
  INTENT_PATTERNS: {
    transactional: [
      'buy', 'purchase', 'order', 'shop', 'price', 'cost', 'deal', 'discount', 
      'coupon', 'free', 'download', 'signup', 'register', 'subscribe', 'deposit', 
      'withdraw', 'bet', 'wager', 'play', 'claim', 'bonus', 'checkout', 'cart',
      'sale', 'offer', 'promo', 'trial', 'demo', 'quote', 'pricing', 'plans'
    ],
    commercial: [
      'best', 'top', 'review', 'comparison', 'compare', 'vs', 'versus', 
      'alternative', 'recommended', 'rating', 'winner', 'trusted', 'safe', 
      'legit', 'licensed', 'regulated', 'affordable', 'cheap', 'premium',
      'enterprise', 'professional', 'features', 'benefits', 'pros', 'cons'
    ],
    informational: [
      'how', 'what', 'why', 'when', 'where', 'who', 'which', 'guide', 
      'tutorial', 'learn', 'tips', 'strategy', 'method', 'step', 'introduction', 
      'beginner', 'basics', 'explain', 'definition', 'meaning', 'example',
      'template', 'checklist', 'resource', 'course', 'training', 'certification'
    ],
    navigational: [
      'login', 'signin', 'sign in', 'log in', 'account', 'dashboard', 'profile', 
      'official', 'website', 'contact', 'support', 'help', 'faq', 'documentation',
      'status', 'careers', 'jobs', 'about', 'team', 'blog', 'news'
    ]
  },
  
  // NEW: Search volume estimation patterns
  VOLUME_INDICATORS: {
    high: ['best', 'top', 'how to', 'what is', 'vs', 'review', 'price', 'free'],
    medium: ['guide', 'tutorial', 'tips', 'comparison', 'alternative', 'example'],
    low: ['specific', 'advanced', 'niche', 'technical', 'enterprise', 'api']
  },
  
  // NEW: Difficulty indicators
  DIFFICULTY_INDICATORS: {
    high: ['best', 'top', 'review', 'vs', 'comparison'],
    medium: ['how to', 'guide', 'tutorial', 'tips'],
    low: ['specific', 'niche', 'long-tail', 'question']
  }
};
