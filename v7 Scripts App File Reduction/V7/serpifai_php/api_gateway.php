...
// Ensure fatal errors produce JSON instead of an empty response
register_shutdown_function(function() {
    ...
    if ($err && in_array($err['type'], [E_ERROR, E_PARSE, E_CORE_ERROR, E_COMPILE_ERROR])) {
        ...
        // Do not call exit() in shutdown function - script is terminating
    }
});
...
function executeAction($action, $payload, $user, $license) {
    ...
    // ═══════════════════════════════════════════════════════════════════════════
    // ELITE DATA ORCHESTRATION ENDPOINTS (v2.0)
    // 10X Superior Data Collection vs Competitors
    // ═══════════════════════════════════════════════════════════════════════════
    
    // ELITE Multi-Source Data Collection
    if (strpos($action, 'elite:collect') === 0 || strpos($action, 'elite_collect') === 0) {
        require_once __DIR__ . '/handlers/elite_data_handler.php';
        return handleEliteCollectAction($action, $payload, $license, $user['id']);
    }
    
    // ELITE Cross-Validation
    if (strpos($action, 'elite:validate') === 0 || strpos($action, 'elite_validate') === 0) {
        require_once __DIR__ . '/handlers/elite_data_handler.php';
        return handleEliteValidateAction($action, $payload, $license, $user['id']);
    }
    
    // ELITE Data Quality Check
    if (strpos($action, 'elite:quality') === 0 || strpos($action, 'elite_quality') === 0) {
        require_once __DIR__ . '/handlers/elite_data_handler.php';
        return handleEliteQualityAction($action, $payload, $license, $user['id']);
    }
    
    // ELITE Cache Management
    if (strpos($action, 'elite:cache') === 0 || strpos($action, 'elite_cache') === 0) {
        require_once __DIR__ . '/handlers/elite_data_handler.php';
        return handleEliteCacheAction($action, $payload, $license, $user['id']);
    }
    
    // ELITE Confidence Scoring
    if (strpos($action, 'elite:confidence') === 0 || strpos($action, 'elite_confidence') === 0) {
        require_once __DIR__ . '/handlers/elite_data_handler.php';
        return handleEliteConfidenceAction($action, $payload, $license, $user['id']);
    }
    }
