...
function calculateCreditCost($action, $payload) {
    ...
    if (strpos($action, 'workflow:stage') === 0 || strpos($action, 'workflow_stage') === 0) {
        ...
        $freeCompActions = ['comp:save_results', 'comp:load_results', 'comp:list_projects', 'comp:delete_results', 'comp:save_trend', 'comp:get_trend', 'comp:get_trends_bulk', 'comp:refresh_metric', 'comp:get_consistency', 'comp:fetch_priority'];
