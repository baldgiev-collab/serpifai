...
const GATEWAY_CONFIG = {
  ...
  // Security - MUST match server .env HMAC_SECRET
...
function callGateway(action, payload, licenseKey) {
  ...
  try {
    ...
    const isUserAction = userActions.indexOf(action) !== -1;
    ...
    // Prepare HTTP options (plain JSON, no security for now)
    const options = {
      method: 'post',
    ...
    let lastError;
    for (var attempt = 1; attempt <= GATEWAY_CONFIG.MAX_RETRIES; attempt++) {
      try {
        ...
        try {
          ...
          Logger.log('❌ Action that failed: ' + action);
          Logger.log('❌ Gateway URL: ' + gatewayUrl);
          Logger.log('❌ Response code: ' + responseCode);
          Logger.log('❌ Raw response (first 500 chars): ' + responseText.substring(0, 500));
          Logger.log('❌ Raw response (last 200 chars): ' + responseText.substring(Math.max(0, responseText.length - 200)));
          throw new GatewayError('Invalid JSON response from gateway for action "' + action + '" (code: ' + responseCode + ', length: ' + responseText.length + '): ' + responseText.substring(0, 200));
        }
