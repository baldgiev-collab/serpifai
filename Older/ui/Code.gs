...
function runEliteCompetitorAnalysis(competitors, projectContext) {
  try {
    ...
    // Get the active spreadsheet ID (with null check for web app mode)
    let spreadsheetId = 'webapp-session-' + Date.now();
    try {
      ...
      Logger.log('⚠️ No active spreadsheet (running as web app): ' + e);
    }
