...
function DB_COMP_executeEliteAnalysis(config) {
  ...
  Logger.log('🎯 ELITE Competitor Analysis Starting...');
  ...
  try {
    ...
    let competitorData;
    if (useParallel) {
      // V8.1: Parallel fetch with caching - ALWAYS include PageSpeed for complete data
      // PageSpeed provides critical SEO/Performance scores that UI needs
      const fetchOptions = {
        skipPageSpeed: config.skipPageSpeed === true, // Default to INCLUDE PageSpeed (false)
    ...
    // Step 4: Save results to MySQL and MASTER Google Sheet
    Logger.log('💾 Step 4: Saving to master database...');
    const saveResults = saveCompetitorResults(enrichedData, analysis, projectId, config);
...
function fetchAllCompetitorData(competitors) {
  ...
  competitors.forEach((domain, index) => {
    ...
    try {
      ...
      // Rate limiting - reduced for better performance while respecting APIs
      Utilities.sleep(400);
...
function generateGeminiAnalysis(competitorData, yourDomain, projectContext) {
  ...
  // ═══════════════════════════════════════════════════════════════════════
  // GEMINI CALL WITH RETRY LOGIC FOR TRUNCATION HANDLING
  ...
  //   2. Retry up to 2 times if incomplete
  //   3. Use increased token limit (32K) for retries
  //   4. Accept partial response if all retries fail
  // ═══════════════════════════════════════════════════════════════════════
...
/**
 * Save competitor results to MySQL and MASTER Google Sheet
 * Uses ONE central spreadsheet for ALL projects
 * NOTE: MySQL save may fail due to mod_security blocking large payloads - this is OK
 */
function saveCompetitorResults(competitorData, analysis, projectId, config) {
  ...
  // DEFENSIVE: Validate inputs
  if (!competitorData || typeof competitorData !== 'object') {
    Logger.log('⚠️ Invalid competitor data for saving');
    competitorData = {};
  }
  
  if (!analysis || typeof analysis !== 'object') {
    Logger.log('⚠️ Invalid analysis for saving');
    analysis = { text: 'No analysis available', error: 'Invalid analysis object' };
  }
  
  projectId = projectId || 'comp-' + new Date().getTime();
  config = config || {};
  
  // Extract spreadsheetId from config if available
  const spreadsheetId = config.spreadsheetId || null;
  Logger.log('   📊 Project ID: ' + projectId);
  
  // Prepare minimal data package for MySQL (to avoid mod_security blocks)
  const minimalData = {
    projectId: projectId,
    timestamp: new Date().toISOString(),
  ...
  // 1. Save to MySQL via PHP Gateway (OPTIONAL - may fail due to mod_security)
  // NOTE: This is non-critical - Sheet save is the primary storage
  try {
    const mysqlResult = callGateway('comp:save_results', {
      projectId: projectId,
      data: JSON.stringify(minimalData), // Use minimal data to avoid blocks
      competitors: Object.keys(competitorData),
      yourDomain: config.yourDomain || 'Your Site',
      metadata: {
        competitorCount: Object.keys(competitorData).length,
        dataQuality: 'elite',
        apiSuccess: '5/5'
    ...
    if (mysqlResult && mysqlResult.success) {
      results.mysql = {
        success: true,
        recordId: mysqlResult.recordId || mysqlResult.id
      };
      Logger.log('   ✅ MySQL saved (ID: ' + results.mysql.recordId + ')');
    } else {
      results.mysql = {
        success: false,
        error: mysqlResult ? mysqlResult.error : 'No response'
      };
      Logger.log('   ⚠️ MySQL save skipped (not critical): ' + (mysqlResult ? mysqlResult.error : 'No response'));
    }
  } catch (e) {
    // MySQL save failure is NOT critical - we have Sheet backup
    results.mysql = {
      success: false,
      error: e.toString(),
      note: 'MySQL save is optional - Sheet save is primary'
    };
    Logger.log('   ⚠️ MySQL save skipped: ' + e.toString().substring(0, 100));
  }
  
  // 2. Save to MASTER Google Sheet (ONE central sheet for everything)
  try {
    ...
    if (sheetResult && sheetResult.success) {
      Logger.log('   ✅ Master Sheet saved');
      if (sheetResult.url) {
        Logger.log('      🔗 URL: ' + sheetResult.url);
      }
    } else {
      Logger.log('   Sheet save error: ' + (sheetResult ? sheetResult.error : 'No result'));
    }
  } catch (e) {
    results.sheets = {
      success: false,
  ...
  Logger.log('   ✅ Storage complete - MySQL: ' + results.mysql.success + ', Sheet: ' + results.sheets.success);
