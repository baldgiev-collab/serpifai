...
function analyzePageSpeed($url, $strategy = 'mobile') {
    ...
    // Build API URL - use array for multiple categories
    $params = [
        'url' => $url,
    ...
    $categoryParams = 'category=performance&category=accessibility&category=best-practices&category=seo';
    $apiUrl = 'https://www.googleapis.com/pagespeedonline/v5/runPagespeed?' . http_build_query($params) . '&' . $categoryParams;
    
    // Check cache (but validate the cached data has real scores)
    $cacheKey = 'pagespeed_' . md5($url . '_' . $strategy);
    $cached = getCacheValue($cacheKey);
    if ($cached) {
        // VALIDATE: Only use cache if ALL scores are present and performance > 0
        $perfScore = $cached['scores']['performance'] ?? 0;
        $accessScore = $cached['scores']['accessibility'] ?? 0;
        $bpScore = $cached['scores']['best_practices'] ?? 0;
        $seoScore = $cached['scores']['seo'] ?? 0;
        
        // Calculate total - if we have real data, total should be > 100
        $totalScore = $perfScore + $accessScore + $bpScore + $seoScore;
        
        // ONLY use cache if performance > 0 AND total > 100 (meaning we have real data)
        // This invalidates any cached results with zeros in performance
        if ($perfScore > 0 && $totalScore > 100) {
            $cached['cached'] = true;
            error_log("PageSpeed cache HIT for $url - using cached data (perf=$perfScore, total=$totalScore)");
            return $cached;
        }
        // Otherwise, skip cache and make fresh API call
        error_log("PageSpeed cache INVALID for $url - refetching (perf=$perfScore, access=$accessScore, bp=$bpScore, seo=$seoScore, total=$totalScore)");
    ...
    $result = [
        ...
        'scores' => [
            ...
            'best_practices' => ($data['lighthouseResult']['categories']['best-practices']['score'] ?? 0) * 100,
